...
BEGIN
  ...
  WITH account_balances AS (
    SELECT 
      ...
      coa.code::text,
      coa.name,
      COALESCE(SUM(CASE WHEN t.debit > 0 AND h.transaction_date <= p_date AND h.status = 'posted' THEN t.debit ELSE 0 END), 0) as total_debit,
      COALESCE(SUM(CASE WHEN t.credit > 0 AND h.transaction_date <= p_date AND h.status = 'posted' THEN t.credit ELSE 0 END), 0) as total_credit
